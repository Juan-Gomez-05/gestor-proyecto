...
export default function Dashboard() {
  ...
  const tareasFiltradas = useMemo(() => {
    ...
    if (nombreRol === "ADMINISTRADOR") tareasBase = tareas;
    else if (nombreRol === "GERENTE") {
      tareasBase = tareas.filter((t) => {
        const proyecto = proyectos.find((p) => p.ProyectoID === t.ProyectoID);
        return proyecto && proyecto.area === areaUsuario;
      });
    } else if (nombreRol === "LIDER") {
      tareasBase = tareas.filter((t) => {
        const proyecto = proyectos.find((p) => p.ProyectoID === t.ProyectoID);
        return proyecto && proyecto.UsuarioPropietarioID === idUsuario;
      });
    } else {
      tareasBase = tareas.filter((t) =>
    ...
    // Filtro adicional por proyectos seleccionados
    ...
    return tareasBase;
  }, [tareas, proyectos, nombreRol, idUsuario, areaUsuario, selectedProjects]);

  const proyectosFiltrados = useMemo(() => {
    if (!Array.isArray(proyectos)) return [];
    if (nombreRol === "ADMINISTRADOR") return proyectos;
    if (nombreRol === "GERENTE")
      return proyectos.filter((p) => p.area === areaUsuario);
    if (nombreRol === "LIDER")
      return proyectos.filter((p) => p.UsuarioPropietarioID === idUsuario);
    return proyectos.filter((p) =>
      tareasFiltradas.some((t) => t.ProyectoID === p.ProyectoID)
    );
  }, [proyectos, tareasFiltradas, nombreRol, idUsuario, areaUsuario]);
