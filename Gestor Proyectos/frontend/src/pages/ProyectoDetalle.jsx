...
import {
  Spin, Tabs, Typography, Space, Input, Select, Button,
  Collapse, Table, Tag, Avatar, Tooltip, DatePicker,
  Modal, Form, message,
} from 'antd';
...
import {
  ...
  CalendarOutlined, OrderedListOutlined,
...
const prioridadMeta = { Baja:'#90BC1F', Media:'#EE8D00', Alta:'#662480', Crítica:'#000' };

export default function ProyectoDetalle() {
  const { id } = useParams();
  const [filtro, setFiltro] = useState({ texto:'', persona:null, estado:null });
  ...
  const { data: tareas = [], isLoading: lt, refetch } = useQuery(['tareas', id], () =>
...
, [estadosTarea]);
  ...
  const crearM = useMutation(datos => api.post('/tareas/', datos), {
    onSuccess: () => { refetch(); message.success('Tarea creada'); }
  });
  const editarM = useMutation(datos =>
    api.put(`/tareas/${datos.tarea_id}`, datos), {
      onSuccess: () => { refetch(); message.success('Tarea actualizada'); }
    }
  );
  const borrarM = useMutation(idT => api.delete(`/tareas/${idT}`), {
    onSuccess: () => { refetch(); message.success('Tarea eliminada'); }
  });

  const tareasFiltradas = useMemo(() => {
    if ([1,2,3].includes(idRol)) return tareas;
    return tareas.filter(t =>
      Array.isArray(t.usuario_asignados) && t.usuario_asignados.includes(idUsuario)
    );
  }, [idRol, idUsuario, tareas]);

  const participantesProyecto = useMemo(() =>
    usuarios.filter(u =>
      tareasFiltradas.some(t => (t.usuario_asignados || []).includes(u.UsuarioID))
  ...
  const datosFiltrados = useMemo(() =>
    tareasFiltradas
      .filter(t => !filtro.texto || t.Titulo.toLowerCase().includes(filtro.texto.toLowerCase()))
      .filter(t => !filtro.persona || (t.usuario_asignados||[]).includes(filtro.persona))
...
const ganttTasks = tareasParaGantt.map(t => ({
  id: String(t.TareaID),
...
const events = tareasFiltradas.map(t => ({
  ...
  render: text => (
        ...
        <Space>
          {(arr || []).map(uid => (
            <Tooltip key={uid} title={lookupName(uid)}>
              <Avatar size="small">
                {lookupName(uid).split(' ').map(n => n[0]).join('')}
              </Avatar>
            </Tooltip>
          ))}
    ...
    },
    {
      title:'Estado',
      dataIndex:'Estado',
      filters: ordenEstados.map(e=>({ text: estadosMeta[e].label, value: e })),
      onFilter:(val,row)=>row.Estado===val,
      width: 100,
      render:e=><Tag color={estadosMeta[e].color}>{estadosMeta[e].label}</Tag>
    },
    {
      title: 'Antecesora',
  ...
  filterDropdown: ({ setSelectedKeys, selectedKeys, confirm, clearFilters }) => (
    <div style={{ padding: 8 }}>
      ...
      <Space>
        ...
        <Button onClick={() => { clearFilters(); confirm({ closeDropdown: true }); }} size="small" style={{ width: 90 }}>
          Limpiar
        </Button>
      </Space>
    </div>
  ...
  filterIcon: filtered => <SearchOutlined style={{ color: filtered ? '#1890ff' : undefined }} />,
  onFilter: (value, record) =>
    record.FechaInicio?.startsWith(value)
},
{
  title: 'Vencimiento',
  ...
  filterIcon: filtered => <SearchOutlined style={{ color: filtered ? '#1890ff' : undefined }} />,
  onFilter: (value, record) =>
    record.FechaLimite?.startsWith(value)
},
    {
  title: 'Prioridad',
  dataIndex: 'prioridad',
      ...
      <Space>
        <Button size="small" onClick={() => { 
          ...
          editForm.setFieldsValue({
            ...
            comentarios: parseComentarios(rec.comentarios).map(c => `${c.nombre} (${dayjs(c.fecha).format("YYYY-MM-DD HH:mm")}): ${c.texto}`).join('\n'),
            antecesora: rec.antecesora || null
          });
          setEditModalVisible(true);
        }}>Editar</Button>
        <Button danger size="small" onClick={() => borrarM.mutate(rec.TareaID)}>Eliminar</Button>
      </Space>
    </div>
  )
}
  ...
  return (
    <div style={{ padding:24 }}>
      ...
      <Tabs defaultActiveKey="1" type="card" tabBarGutter={20}>
        <Tabs.TabPane tab={<span><TableOutlined /> Tabla</span>} key="1">
          <Collapse accordion>
            {ordenEstados.map(est=>{
              const grupo = datosFiltrados.filter(t=>t.Estado===est);
              if (!grupo.length) return null;
              return (
                <Panel key={est} header={<Text strong style={{ color:estadosMeta[est].color }}>{estadosMeta[est].label} ({grupo.length})</Text>}>
                    ...
                    rowSelection={{
                      ...
                      onSelect: (record, selected) => {
                        setSeleccionadas(prev => {
                          const existe = prev.some(t => t.TareaID === record.TareaID);
                          if (selected && !existe) return [...prev, record];
                          if (!selected) return prev.filter(t => t.TareaID !== record.TareaID);
                          return prev;
                        });
                      },
                      onSelectAll: (selected, selectedRows, changeRows) => {
                        const allSelectedIds = tareasFiltradas.map(t => t.TareaID);
                        ...
                        if (allAlreadySelected) {
                          // Deselect all
                          setSeleccionadas(seleccionadas.filter(t => !allSelectedIds.includes(t.TareaID)));
                        } else {
                          // Select all (sin duplicar)
                          const nuevos = tareasFiltradas.filter(
                            t => !seleccionadas.some(s => s.TareaID === t.TareaID)
                          );
                  ...
                  <Space style={{ marginTop:16 }} wrap>
                    <Input placeholder="Título" style={{ width:160 }} value={nueva.__new?.Titulo} onChange={e=>setNueva(n=>({...n, __new:{...n.__new,Titulo:e.target.value}}))}/>
                    <DatePicker placeholder="Inicio" value={nueva.__new?.FechaInicio} onChange={d=>setNueva(n=>({...n, __new:{...n.__new,FechaInicio:d}}))}/>
                    <DatePicker placeholder="Fin" value={nueva.__new?.FechaLimite} onChange={d=>setNueva(n=>({...n, __new:{...n.__new,FechaLimite:d}}))}/>
                    <Select placeholder="Prioridad" style={{ width:100 }} value={nueva.__new?.prioridad} onChange={v=>setNueva(n=>({...n, __new:{...n.__new,prioridad:v}}))}>
                      {Object.keys(prioridadMeta).map(p=><Option key={p} value={p}>{p}</Option>)}
                    </Select>
                    <Select mode="multiple" placeholder="Responsables" style={{ width:180 }} value={nueva.__new?.usuario_asignados} onChange={arr=>setNueva(n=>({...n, __new:{...n.__new,usuario_asignados:arr}}))}>
                      {usuarios.map(u=><Option key={u.UsuarioID} value={u.UsuarioID}>{u.NombreCompleto}</Option>)}
                    </Select>
                    <Button type="primary" onClick={handleCreate}>Guardar</Button>
                  </Space>
                </Panel>
              );
            })}
    ...
    <Button type="primary" onClick={handleCreate}>Guardar</Button>
        ...
        <Tabs.TabPane tab={<span><ProjectOutlined /> Gantt</span>} key="2">
          <Select
        ...
        <Tabs.TabPane tab={<span><BarChartOutlined /> Gráfico</span>} key="3">
          <ChartView tasks={tareasFiltradas} usuarios={usuarios}/>
        </Tabs.TabPane>

        <Tabs.TabPane tab={<span><CalendarOutlined /> Calendario</span>} key="4">
          <CalendarView events={events}/>
        </Tabs.TabPane>

        <Tabs.TabPane tab={<span><OrderedListOutlined /> Kanban</span>} key="5">
          <KanbanView  tasks={tareasFiltradas} usuarios={usuarios}/>
        </Tabs.TabPane>

        {[1, 2, 3].includes(idRol) && (
          <>
            <Tabs.TabPane tab={<span><UserOutlined /> Recursos</span>} key="6">
              <RecursosView tareas={tareasFiltradas} usuarios={participantesProyecto} />
            </Tabs.TabPane>
            <Tabs.TabPane tab={<span><DollarOutlined /> Financiero</span>} key="7">
              <FinancieroView tareas={tareasFiltradas} usuarios={participantesProyecto} roles={roles} />
            </Tabs.TabPane>
          </>
        )}
      </Tabs>
        ...
        onOk={async () => {
          try {
            ...
            editarM.mutate({
              ...
              prioridad: { Baja: 1, Media: 2, Alta: 3, Crítica: 4 }[vals.prioridad] || 2,
      ...
      >
        <Form form={editForm} layout="vertical">
          ...
          <Form.Item name="Estado" label="Estado" rules={[{ required:true }]}><Select>{ordenEstados.map(e=><Option key={e} value={e}>{estadosMeta[e].label}</Option>)}</Select></Form.Item>
          <Form.Item name="PorcentajeAvance" label="Avance (%)"><Input type="number" min={0} max={100}/></Form.Item>
          <Form.Item name="prioridad" label="Prioridad"><Select>{Object.keys(prioridadMeta).map(p=><Option key={p} value={p}>{p}</Option>)}</Select></Form.Item>
          <Form.Item name="FechaInicio" label="Fecha Inicio" rules={[{ required:true }]}><DatePicker/></Form.Item>
          <Form.Item name="FechaLimite" label="Fecha Límite" rules={[{ required:true }]}><DatePicker/></Form.Item>
          <Form.Item name="usuario_asignados" label="Responsables"><Select mode="multiple">{usuarios.map(u=><Option key={u.UsuarioID} value={u.UsuarioID}>{u.NombreCompleto}</Option>)}</Select></Form.Item>
          ...
          <Form.Item name="antecesora" label="Antecesora">
            <Select allowClear placeholder="Seleccione antecesora">
              ...
              {tareas.filter(t => t.TareaID !== editingTask?.TareaID).map(t => (
                <Option key={t.TareaID} value={t.TareaID}>{t.Titulo}</Option>
              ))}
            </Select>
          </Form.Item>
        </Form>
...
<Modal
  ...
  onOk={async () => {
    ...
    const usuarios = destinatariosSeleccionados.filter(v => !v.includes('@')).map(v => parseInt(v));
    const con_copia = destinatariosSeleccionados.filter(v => v.includes('@'));
...
>
  ...
  onOk={async () => {
    ...
    const entrada = `[${user.Nombre};${dayjs().format("YYYY-MM-DD HH:mm:ss")};${nuevoComentario}]`;
    ...
    await api.put(`/tareas/${comentarioTareaSeleccionada.TareaID}`, {
      ...
      prioridad: prioridadTextoANumero[comentarioTareaSeleccionada.prioridad] || 2,
      fecha_inicio: comentarioTareaSeleccionada.FechaInicio,
      fecha_limite: comentarioTareaSeleccionada.FechaLimite,
      usuario_asignados: comentarioTareaSeleccionada.usuario_asignados || [],
      comentarios: (comentarioTareaSeleccionada.comentarios || '') + entrada,
...
>
  ...
  <div style={{ flex: 1, overflowY: 'auto', paddingRight: 8, marginBottom: 10 }}>
    {parseComentarios(comentarioTareaSeleccionada?.comentarios).map((c, i) => (
      <div key={i} style={{ marginBottom: 8, padding: 6, borderRadius: 6, background: '#f5f5f5' }}>
